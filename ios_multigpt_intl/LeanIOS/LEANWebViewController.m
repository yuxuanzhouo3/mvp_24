...
- (void) handleJSBridgeFunctions:(id)data{
    ...
    if ([url.scheme.lowercaseString isEqualToString:@"median"] &&
        [url.host.lowercaseString isEqualToString:@"iap"] &&
        [url.path.lowercaseString isEqualToString:@"/purchase"]) {
        NSString *productId = query[@"productId"] ?: query[@"product_id"];
        NSString *callback = query[@"callback"] ?: query[@"callbackFunction"];

        // Support fallback scheme: median://iap/purchase?payload=<base64(json)>&callback=...
        // The web layer may send productId inside payload instead of as a query param.
        NSString *payload = query[@"payload"];
        if ((!productId || productId.length == 0) && payload && payload.length > 0) {
            NSData *decodedData = [[NSData alloc] initWithBase64EncodedString:payload options:0];
            if (decodedData) {
                id obj = [NSJSONSerialization JSONObjectWithData:decodedData options:0 error:nil];
                if ([obj isKindOfClass:[NSDictionary class]]) {
                    NSDictionary *payloadDict = (NSDictionary *)obj;
                    NSString *pid = payloadDict[@"productId"] ?: payloadDict[@"product_id"];
                    if (pid && [pid isKindOfClass:[NSString class]] && pid.length > 0) {
                        productId = pid;
                    }
                    NSString *cb = payloadDict[@"callback"] ?: payloadDict[@"callbackFunction"];
                    if ((!callback || callback.length == 0) && cb && [cb isKindOfClass:[NSString class]] && cb.length > 0) {
                        callback = cb;
                    }
                }
            }
        ...
        if (!productId || productId.length == 0) {
            if (callback && callback.length > 0) {
                [self runJavascriptWithCallback:callback data:@{ @"status": @"fail", @"message": @"Missing productId" }];
            }
            return;
        }

        [[IAPManager shared] purchaseWithProductId:productId callback:^(NSDictionary * _Nonnull data) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (callback && callback.length > 0) {
                    [self runJavascriptWithCallback:callback data:data];
                }
            });
        }];
        return;
