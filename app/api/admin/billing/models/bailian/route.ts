...
import { fetchBailianBillingImportItems } from "@/lib/importers/bailian";
...
function ensureCn(session: Awaited<ReturnType<typeof getAdminSession>>) {
  ...
  if (session.region !== "CN") {
    return NextResponse.json({ success: false, error: "仅国内版后台支持阿里百炼导入" }, { status: 400 });
  }
  return null;
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const data = await fetchBailianBillingImportItems({ limit });
    return NextResponse.json({ success: true, data });
  } catch (error) {
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "阿里百炼抓取失败" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const fetched = await fetchBailianBillingImportItems({ limit });
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "阿里百炼导入失败" },
      { status: 500 }
    );
  }
}
