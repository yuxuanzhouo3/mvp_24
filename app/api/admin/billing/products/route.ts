...
import {
  listPaymentProductEntries,
  upsertPaymentProductEntries,
...
export async function GET() {
  ...
  const data = await listPaymentProductEntries(session.region);
...
export async function POST(req: NextRequest) {
  ...
  const result = await upsertPaymentProductEntries(items, session.region);
  if (!result.success) {
