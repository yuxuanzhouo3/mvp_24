...
export function AICollaborationPanel({ selectedGPTs, language, isProcessing }: AICollaborationPanelProps) {
  const t = {
    zh: {
      collaboration: "协作状态",
      processing: "处理中",
    ...
    <Card className="p-4 bg-gradient-to-r from-blue-50 to-purple-50 border-blue-200">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="font-semibold text-blue-900">{t[language].collaboration}</h3>
        </div>
        {isProcessing && (
          <Badge variant="secondary" className="bg-blue-100 text-blue-800 animate-pulse">
            {t[language].autoWorking}
          </Badge>
        )}
      </div>
              ...
              <div className="flex-1">
                <div className="flex items-center space-x-2">
                  ...
                  {gpt.type === "organization" && (
                    <Badge variant="outline" className="text-xs">
                      {t[language].members}: {gpt.members?.length || 0}
                    </Badge>
                  )}
                </div>
                ...
                {gpt.type === "organization" && (
                  <div className="text-xs text-gray-600 mt-1">
                    {language === "zh" ? "自动协调内部成员工作" : "Auto-coordinating internal member work"}
                  </div>
                )}

                {status === "processing" && <Progress value={65} className="w-full h-2 mt-2" />}
              </div>

              {index < selectedGPTs.length - 1 && <ArrowRight className="w-4 h-4 text-gray-400" />}
            </div>
          )
        })}
      </div>
      ...
      {isProcessing && (
        <div className="mt-4 p-3 bg-white/50 rounded-lg">
          <div className="text-sm text-gray-700">
            {language === "zh" ? "🤖 AI团队正在协作处理您的请求..." : "🤖 AI team is collaborating on your request..."}
          </div>
        </div>
      )}
    </Card>
